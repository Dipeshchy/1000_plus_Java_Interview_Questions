...
A thread in Java is a lightweight process that runs within another
process or thread.
...
multithreading
3. Reduced number of Servers required: With Multithreading
we need lesser number of servers, since one
...
A thread lives within a process and shares the resources likememory,
open files etc. with the other threads of the same process.
...
stopped by the Java application at runtime.
...
We can use the keyword volatile for instance to make this threadsafe
code.
...
Livelock is a scenario in which two or more block each other by
...
311. Check if following code is threadsafe
for retrieving an integer value
from a Queue?
...
concurrency issues. But it may not behave correctly in a multithreading
scenario.
...
It is applicable in scenario when one lock is used to
...
One of the schedule() method provides the ability to schedule a oneshot
task that can be executed after given delay.
...
Semaphore class in Java is used to implement a counting
...
When current implementation is not already running in a multithread
environment. If there is already a multi-threading
...
1. Immutable-value classes: The new API avoids threadsafety
and concurrency issues by ensuring that all the core
...
not have to deal with concurrency issues while writing datehandling
code.
...
415. How can you access a nonstatic
variable from the static context?
...
This module also supports web-oriented integration features liketransparently
handling multipart requests for uploading files,
...
lifecycle of a WebSocket. It is also valid in the context of a webaware
Spring ApplicationContext.
...
behavior of a singleton bean. Developer is responsible for
...
including <context:annotation-config /> in bean configuration file.
...
is used in web applications. It provides features to deal with webrelated
components like- controllers, view resolvers etc.
...
one database. It is a thread-safe interface that works well in multithreaded
applications.
...
Query: This interface encapsulates the behavior of an objectoriented
query in Hibernate. It can accept parameters and execute
...
<column name="EMPLOYEE_ID" length="10" notnull="
true" unique="true" />
</property>
</class>
...
Metrics: In Hibernate we can also collect other metrics likenumber
of open sessions, retrieved JDBC connections, cache hit,
miss etc.
...
has plugins that are reusable across multiple projects.
...
284. What is a POM?
POM is an abbreviation for Project Object Model. This is the basic
...
and creating the application. It has multiple phases likecompile,
test, package inside it.
...
A multi-module project is created to manage a group of other subprojects
or modules. The multi-module relationship is like a tree
that starts from the topmost level to the bottom level. In a multimodule
project, we specify that a project should include the
...
327. What is GIT?
GIT is a mature Distributed Version Control System (DVCS). It is
...
335. What are the different ways to
start work in GIT?
...
337. What does ‘git pull’ command
...
the code and then merging it into develop or release branch.
...
create multiple commits out of that.
...
AWS Region.
Within a region, Availability Zones are connected through lowlatency
links.
...
Software as Service is a category of cloud computing in which Software is
centrally hosted and it is licensed on a subscription basis. It is also known as
On-demand software. Generally, clients access the software by using a thinclient
like a web browser.
...
files that are two levels down in a subdirectory.
...
With rm –r * command we can delete the file a.txt as well as subdirectories
Level_1_dir and Level_2_dir.
Command:
rm – r *
...
directory on file system. It contains information about file likelocation
of file on the disk, access mode, ownership, file type etc.
...
A Microservice is a small and autonomous piece of code that does
...
496. What are the issues in using
REST over HTTP for Microservices?
...
overhead of serialization, deserialization in HTTP.
497. Can we create Microservices
as State Machines?
Yes, Microservices are independent entities that serve a specific
